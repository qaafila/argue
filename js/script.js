...
document.addEventListener('DOMContentLoaded', (event) => {
    ...
    // For essay structure page
    const essayStructureContainer = document.getElementById('essay-structure');
    if (essayStructureContainer) {
        initDragAndDrop();
        document.getElementById('check-structure').addEventListener('click', checkEssayStructure);
    }

    // Toggle examples on fundamentals page
    ...
    toggleButtons.forEach(button => {
        button.addEventListener('click', function() {
            ...
            this.textContent = example.classList.contains('hidden') ? 'Show Example' : 'Hide Example';
        });
    });
});
